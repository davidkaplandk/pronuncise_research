...
OUT_DIR   = BASE / "hf_wav2vec2_de_phonemes_with_spaces"  # <-- new output
TARGET_SR = 16000

WORD_DELIM = "|"  # CTC word boundary token
...
# -----------------
# TextGrid -> phoneme tokens WITH word boundaries
# -----------------
def load_phones_with_word_boundaries(
    tg_path: Path,
    tier_name: str = "KAN-MAU",
    word_delim: str = WORD_DELIM,
):
    ...
    # Each interval == one "word" (label contains phones for that word: "ɡ eː ɡ ə n")
    words_as_phone_lists = []
    for (_, _, label) in tier.entries:
        lab = (label or "").strip()
        if lab == "":
            continue
        phones = lab.split()  # phones for that word
        if phones:
            words_as_phone_lists.append(phones)

    if not words_as_phone_lists:
        raise ValueError(f"{tg_path}: extracted 0 words/phone-groups from {tier_name}")

    # Flatten and insert WORD_DELIM between words
    out = []
...
def add_phonemes(batch):
    batch["phonemes"] = load_phones_with_word_boundaries(
        Path(batch["grid_path"]),
        tier_name="KAN-MAU",
...
print("Example tokens:", train_ds[0]["phonemes"][:60])
...
# Ensure delimiter is included even if rare
all_phones.add(WORD_DELIM)
...
tokenizer = Wav2Vec2PhonemeCTCTokenizer(
    ...
    word_delimiter_token=WORD_DELIM,   # <-- key: decode WORD_DELIM as space
    pad_token="<pad>",
    unk_token="<unk>",
    bos_token="<s>",
    eos_token="</s>",
...
print("Unknown tokens in vocab mapping:", n_unk)
print("WORD_DELIM id:", tokenizer.convert_tokens_to_ids(WORD_DELIM))
...
def prepare_batch(batch):
    ...
    # labels = ids for phoneme tokens + WORD_DELIM tokens
...
# -----------------
# Metric (PER-ish using WER on token strings)
# -----------------
...
trainer_kwargs = dict(
    ...
    # compute_metrics=compute_metrics,  # can enable if you disable prediction_loss_only
)
...
print(">>> starting training (WITH word boundaries)")
trainer.train()
...
# Quick decode sanity check after training:
# (run after trainer.train())
# pred = trainer.predict(test_ds.select(range(2)))
# pred_ids = np.argmax(pred.predictions, axis=-1)
# print(processor.batch_decode(pred_ids)[0])  # should contain spaces if model learned WORD_DELIM
