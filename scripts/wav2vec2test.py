...
def load_audio(path: Path, sr: int) -> torch.Tensor:
    """
    Load audio with torchaudio, resample to sr, mono, return 1D tensor [T].
    """
    waveform, file_sr = torchaudio.load(path.as_posix())  # [C, T]
...
def transcribe_phonemes(path: Path) -> str:
    """
    Run wav2vec2 phoneme model on one file and return decoded phoneme sequence.
    """
    speech = load_audio(path, target_sr)

    # This mirrors your single-file example
    inputs = processor(
        speech,
    ...
    if hasattr(processor, "batch_decode"):
        ...
        phonemes = processor.tokenizer.batch_decode(pred_ids)
    ...
    with open(out_csv, "a", encoding="utf-8", newline="") as f_out:
        ...
        for wav_path in audio_files:
            ...
            try:
                ...
                preview = phonemes[:80] + ("..." if len(phonemes) > 80 else "")
                print(f"         Phonemes: {preview}")
